...
| 系统托盘图标 | ✅ | 右键菜单直接切换配置（显示开关、日志级别、最大项目数），双语显示 |
...
| 文件夹列表智能更新 | ✅ | 比较新旧列表，数据未变时跳过 UI 更新，避免闪烁 |
...
- **托盘菜单配置**：使用 `MF_CHECKED` 标记显示开关状态，子菜单选择日志级别和最大项目数，菜单项使用中文 | 英文双语显示
...
- **托盘菜单双语显示**：菜单项使用中文 | 英文双语显示，顶部显示版本号 v1.0
- **文件夹列表智能更新**：`UpdateFolders` 比较新旧列表，数据未变时跳过 UI 更新，避免点击文件夹后列表闪烁
...
- **文件夹列表智能更新**：比较新旧列表 Path，数据未变时跳过 `_items.Clear()` + 重建，避免 UI 闪烁
- **导航后跳过刷新**：`_justNavigated` 标记 + 1秒延时重置，导航后跳过 `OnForegroundChanged` 中的 `ShowOverlayForDialog` 调用
